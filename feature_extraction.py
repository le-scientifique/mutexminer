...
splicing_type_dict = {'':-1, 'CERTAIN':1, 'POSSIBLE':0.5}
conservation_dict = {'':-1,'Conserved':1,'Not available':-1, 'MildlyConserved':0.5, 'NotConserved':0}
...
damaging_preds_dict = {'Unknown':-1,'':-1,'Tolerated':0,'Benign':0,'Neutral':0,'PolymorphismAutomatic':0,'Polymorphism':0.1,'Low':0.1,'PossiblyDamaging':0.4,'Medium':0.5,'ProbablyDamaging':0.6,'DiseaseCausing':0.9,'High':0.9,'DiseaseCausingAutomatic':1,'Damaging':1,'Deleterious':1}
...
with open('train.csv') as ip:
	...
	for row in reader:
		...
		row_features.append(Levenshtein.ratio(row[3],row[4]))
...
with open('feature_levenshtein_conts.pkl','w') as op:
